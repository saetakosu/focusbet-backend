...
app.post('/v1/stripe/webhook', async (c) => {
  ...
  switch (event.type) {
    ...
    case 'payment_intent.succeeded': {
      ...
      await pool.query(`
        UPDATE charge_records SET status = 'charged', stripe_payment_intent_id = $1
        WHERE user_id = $2 AND date_string = $3
      `, [pi.id, pi.metadata.userId, pi.metadata.dateString])
      break
    }
    case 'payment_intent.payment_failed': {
      const pi = event.data.object
      await pool.query(`
        UPDATE charge_records SET status = 'failed', failure_reason = $1, retry_count = retry_count + 1
        WHERE user_id = $2 AND date_string = $3
      `, [pi.last_payment_error?.message, pi.metadata.userId, pi.metadata.dateString])
      break
    }
  }
...
async function processJudgment(record) {
  ...
  await pool.query(`
    UPDATE daily_records SET is_achieved = $1, is_judged = TRUE
    WHERE user_id = $2 AND date_string = $3
  `, [isAchieved, record.user_id, record.date_string])

  if (isAchieved) {
    await pool.query(`
      UPDATE daily_records SET charge_status = 'not_required'
      WHERE user_id = $1 AND date_string = $2
    `, [record.user_id, record.date_string])
    return
  }
  ...
  try {
    ...
    await pool.query(`
      UPDATE daily_records SET charge_status = 'charged'
      WHERE user_id = $1 AND date_string = $2
    `, [record.user_id, record.date_string])
    ...
    await pool.query(`
      ...
      ON CONFLICT (user_id, date_string) DO UPDATE SET status = 'failed', failure_reason = $4,
